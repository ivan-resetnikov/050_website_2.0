
class HTMLElement:
    def __init__(self):
        self.parent: HTMLElement|HTMLFactory = None
        self.children: list[HTMLElement] = []

        self.name: str = ""
        self.content: list[HTMLElement]|HTMLElement|str|None = None

        self.attributes: dict = {}
    

    def render_html(self) -> str:
        # NOTE(vanya): Stringify the attributes
        attribute_source: str = ""

        for key, value in self.attributes.items():
            attribute_source += f" {key}=\"{value}\""

        # NOTE(vanya): Render children (Protentially recursive)
        content_html: str = ""

        if isinstance(self.content, str):
            content_html += self.content

        if isinstance(self.content, list):
            for element in p_element.children:
                content_html += element.render_html()

        if isinstance(self.content, HTMLElement):
            content_html += self.content.render_html()

        return f"<{self.name}{attribute_source}>{content_html}</{self.name}>"
    


class HTMLFactory:
    def __init__(self):
        self.elements: list[HTMLElement] = []
        self.root_elements: list[HTMLElement] = []
    

    def push_element(self, p_parent: HTMLElement|None, p_name: str, p_content: None|HTMLElement|str=None, **p_html_attrs) -> HTMLElement:
        e = HTMLElement()

        e.name = p_name
        e.content = p_content

        if isinstance(p_parent, HTMLElement):
            e.parent = p_parent
            p_parent.children.append(e)
        
        elif p_parent == None:
            e.parent = self
            self.root_elements.append(e)

        # NOTE(vanya): Assign attributes
        for key, value in p_html_attrs.items():
            if key == "class_":
                e.attributes["class"] = value
            elif key == "id_":
                e.attributes["id"] = value
            else:
                e.attributes[key] = value
        
        self.elements.append(e)

        return e


    def render_html(self) -> str:
        return "".join([root_element.render_html() for root_element in self.root_elements])